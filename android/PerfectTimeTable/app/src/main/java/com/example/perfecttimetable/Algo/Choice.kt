...
class Choice(con:Context) {
    ...
    fun choiceSubject(area: String, credit: Int, days: List<String>) {
        ...
        var mingap = Chosen.gap
        var temp = Subject("", "", 0, "", 0, "", "")

        days.forEach { day ->
            db.add(DataBase(context).getCreditAreaData(credit, area, day))
        }
    ...
    fun choiceSubjectList(inp: SubjectList, grade: Int) {

        if (inp.title == "전공") {
            Log.d("ChoiceSubjectList", "Selected: 전공")
            choseMajor(grade)
        } else {
            Log.d("ChoiceSubjectList", "Selected: ${inp.title}")
